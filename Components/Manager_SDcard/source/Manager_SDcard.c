...
Std_Err init_manager_SDcard(SDCard_Settings* settings, FIL* file, Motor* motors)
{
    ...
    settings->sdcard_flags.end_program = false;
    settings->sdcard_flags.eofRecieved = false;
    settings->sdcard_flags.executing_program = false;
    settings->sdcard_flags.executing_command = false;
...
Std_Err parse_command_SDcard(SDCard_Settings* settings)
{
    ...
    while(!settings->sdcard_flags.end_program && listSize < 5 &&
        settings->sdcard_flags.executing_program)
    {
        if(settings->cnt >= settings->bytesRead)
        {
            if(settings->sdcard_flags.eofRecieved)
            {
                settings->sdcard_flags.end_program = true;
            ...
            {
                settings->sdcard_flags.eofRecieved = true;
            }
...
Std_Err execute_command_SDcard(SDCard_Settings* settings, bool motors_state)
{
    ...
    if(settings->sdcard_flags.executing_program)
    {
        listSize = fifo_getSize(settings->BuffIN_SDcmd);

        /* no command ongoing, initialize new command */
        if(listSize > 0 && !settings->sdcard_flags.executing_command)
        {
            ...
            settings->sdcard_flags.executing_command = true;
        }

        /* there is command ongoing, process next step */
        if(settings->sdcard_flags.executing_command && executingCmd.step != NULL && motors_state)
        {
